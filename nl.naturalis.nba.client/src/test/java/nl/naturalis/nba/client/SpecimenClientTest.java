...
public class SpecimenClientTest {
	...
	public void testDwcaQueryException()
			throws FileNotFoundException, IOException, NoSuchDataSetException
	{
	...
	public void testDwcaQuery() throws FileNotFoundException, IOException, NoSuchDataSetException
	{
		...
		for (int i = 0; i <= nc.length; i++) {
			FileOutputStream out = new FileOutputStream(
					File.createTempFile("thisismy", "tempfile"));
	...
	public void testgroupByScientificName() throws InvalidQueryException
	{
		...
		QueryResult<ScientificNameGroup> res = client
				.groupByScientificName(new GroupByScientificNameQuerySpec());
		...
		/*
		 * TODO: Move below to integration tests Below test is more of an
		 * integration test and should be moved GroupByScientificNameQuerySpec
		 * qs = new GroupByScientificNameQuerySpec(); String testGenus =
		 * "Alcyonium"; qs.addCondition(new
		 * QueryCondition("identifications.scientificName.genusOrMonomial",
		 * ComparisonOperator.EQUALS, testGenus));
		 * QueryResult<ScientificNameGroup> result =
		 * client.groupByScientificName(qs); for
		 * (Iterator<QueryResultItem<ScientificNameGroup>> it =
		 * result.iterator(); it.hasNext();) { // check if we have found SNGs
		 * ScientificNameGroup sng = it.next().getItem();
		 * assertTrue("ScientificNameGroup object in query result",
		 * sng.getClass() == ScientificNameGroup.class);
		 * 
		 * // check if our genus is in one of the identifications of the
		 * specimen List<Specimen> specimens =
		 * it.next().getItem().getSpecimens(); for (Iterator<Specimen> it2 =
		 * specimens.iterator(); it2.hasNext();) { Specimen sp = it2.next();
		 * boolean genusFound = false; for (Iterator<SpecimenIdentification> it3
		 * = sp.getIdentifications().iterator(); it3.hasNext();) { String
		 * currentGenus = it3.next().getScientificName().getGenusOrMonomial();
		 * if (currentGenus.equals(testGenus)) { genusFound = true; break; } }
