...
/**
 * Provides information about an Elasticsearch document type. This class is very much like an
 * {@code enum}. You cannot instantiate this class. Each {@code DocumentType} instance functions as
 * a little cache of frequently-used, heavy-weight objects associated with the Elasticsearch
 * document type. For example, although you can easily create {@link Mapping} objects yourself, it
 * is recommendable to {@link #getMapping() request} them from the appropriate {@code DocumentType}
 * instance.
 * 
...
public class DocumentType<T extends IDocumentObject> {
  ...
  /**
   * A {@code DocumentType} instance representing the MultiMediaObject document type.
   */
  ...
  static {
    ...
    try {
      for (ConfigObject cfg : getIndexSections()) {
        /*
         * This will set the new IndexInfo instance on the applicable DocumentType instances defined
         * above, and add the applicable DocumentType instances to the new IndexInfo instance.
         */
      ...
      /*
       * No point in going on. We log the error and allow the exception to cause an
    ...
    for (DocumentType<?> one : all) {
      if (one.getIndexInfo() == null) {
        Logger logger = DaoRegistry.getInstance().getLogger(DocumentType.class);
        String fmt = "Missing configurarion for document type %s in %s";
        String msg = String.format(fmt, one.name,
            DaoRegistry.getInstance().getConfigurationFile().getAbsolutePath());
        logger.fatal(msg);
  ...
  /**
   * Returns the {@code DocumentType} instance corresponding to the specified Java class. For aach
   * Elasticsearch document type there is one Java class structured just like it. In fact, the
   * document type is generated through reflection from the Java class.
   */
