...
//			IDENTIFICATIONS_VERNACULAR_NAMES_NAME, GATHERINGEVENT_DATE_TIME_BEGIN,
//			GATHERINGEVENT_SITECOORDINATES_POINT));
//
//	private static final Set<String> specimenNameSearchFieldNames_simpleSearchExceptions = new HashSet<>(
//			Arrays.asList(GATHERINGEVENT_DATE_TIME_BEGIN, GATHERINGEVENT_SITECOORDINATES_POINT));
//
//	private static final Set<String> specimenSearchFieldNames = new HashSet<>(Arrays.asList(
//			UNIT_ID, TYPE_STATUS, PHASE_OR_STAGE, SEX, THEME, COLLECTORS_FIELD_NUMBER,
//			GATHERINGEVENT_LOCALITY_TEXT, GATHERINGEVENT_GATHERING_PERSONS_FULLNAME,
//			GATHERINGEVENT_GATHERING_ORGANISATIONS_NAME, GATHERINGEVENT_DATE_TIME_BEGIN,
//			GATHERINGEVENT_SITECOORDINATES_POINT, COLLECTION_TYPE, /* NDA-386 */
//			SOURCE_SYSTEM_NAME, /* NDA387 */
//			SOURCESYSTEM, IDENTIFICATIONS_SCIENTIFIC_NAME_FULL_SCIENTIFIC_NAME + ".raw")); /* NDA448 */
//
//	private static final Set<String> specimenSearchFieldNames_simpleSearchExceptions = new HashSet<>(
//			Arrays.asList(GATHERINGEVENT_DATE_TIME_BEGIN, GATHERINGEVENT_SITECOORDINATES_POINT));
//
...
//			BioportalTaxonDao bioportalTaxonDao, TaxonDao taxonDao, String baseUrl)
//	{
...
//	 *
//	 * @param params
//	 *            A {@link QueryParams} object containing: 1. fields ... . A
...
//			/*
//			 * If that's what we want, we should first remove any other _andOr
//			 * params
//			 */
...
//		evaluateSimpleSearch(params, specimenSearchFieldNames,
//				specimenSearchFieldNames_simpleSearchExceptions);
//		List<FieldMapping> fields = getSearchParamFieldMapping()
//				.getSpecimenMappingForFields(params);
//		List<FieldMapping> fieldMappings = filterAllowedFieldMappings(fields,
//				specimenSearchFieldNames);
//
//		SearchResponse searchResponse = executeExtendedSearch(params, fieldMappings, SPECIMEN_TYPE,
//				highlighting, sessionId);
//
...
//			minScore = executeExtendedSearch(copy, fieldMappings, SPECIMEN_TYPE, false, sessionId)
//					.getHits().getAt(0).getScore();
//		}
//
...
//	}
//
//	private ResultGroupSet<Specimen, String> doSpecimenNameSearch(QueryParams params,
...
//		if (params.containsKey("_groupMaxResults") || params.containsKey("_groupOffset")
//				|| params.containsKey("_groupSort") || params.containsKey("_groupSortDirection")) {
//			params.remove("_offset");
//		}
//		String sessionId = params.getParam("_session_id");
//		params.remove("_session_id");
//
//		evaluateSimpleSearch(params, specimenNameSearchFieldNames,
//				specimenNameSearchFieldNames_simpleSearchExceptions);
//		List<FieldMapping> fields = getSearchParamFieldMapping()
//				.getSpecimenMappingForFields(params);
//		List<FieldMapping> allowedFields = filterAllowedFieldMappings(fields,
//				specimenNameSearchFieldNames);
//
//		QueryAndHighlightFields nameResQuery = buildNameResolutionQuery(allowedFields,
//				params.getParam("_search"), bioportalTaxonDao, highlighting, getOperator(params),
//				sessionId);
//		
//		SearchResponse searchResponse = executeExtendedSearch(params, allowedFields, SPECIMEN_TYPE,
//				highlighting, nameResQuery, sessionId);
//
//		long totalHits = searchResponse.getHits().getTotalHits();
//		float minScore = 0;
//		if (totalHits > 1) {
//			QueryParams copy = params.copy();
//			copy.add("_offset", String.valueOf(totalHits - 1));
//			SearchHits hits = executeExtendedSearch(copy, allowedFields, SPECIMEN_TYPE, false,
//					sessionId).getHits();
//			try {
...
//	 *
//	 * @param params
//	 *            A {@link QueryParams} object containing: 1. unitID. The unitID
//	 *            of the specimen. 2. _source. An enumerated value representing
//	 *            which method was responsible for generating the result set
//	 *            that contained the currently requested specimen. Valid values:
//	 *            “SPECIMEN_NAME_SEARCH”, “SPECIMEN_SEARCH” and
...
//			ResultGroupSet<Specimen, String> specimenResultGroupSet = doSpecimenNameSearch(params,
//					true);
...
//				"  Invalid value for query parameter \"_source\": \"%s\"", source));
//	}
//
//	// ==================================================== Helpers ====================================================
//
...
//			foundSpecimenForUnitId.addLink(new Link("_previous", SPECIMEN_DETAIL_BASE_URL
//					+ previousSpecimen.getResult().getUnitID()));
//		}
...
//			foundSpecimenForUnitId.addLink(new Link("_next", SPECIMEN_DETAIL_BASE_URL
//					+ nextSpecimen.getResult().getUnitID()));
//		}
//
...
//							List<SearchResult<Specimen>> previousBucket = allBuckets.get(
//									currentBucketIndex - 1).getSearchResults();
...
//							List<SearchResult<Specimen>> nextBucket = allBuckets.get(
//									currentBucketIndex + 1).getSearchResults();
...
//	}
//
//	private SearchResultSet<Specimen> responseToSpecimenSearchResultSet(SearchResponse response,
//			float minScore, String sessionId)
//	{
...
//			ESSpecimen esSpecimen = getObjectMapper().convertValue(hit.getSource(),
//					ESSpecimen.class);
//			Specimen transfer = SpecimenTransfer.transfer(esSpecimen);
//			List<Specimen> specimensWithSameAssemblageId = getOtherSpecimensWithSameAssemblageId(
//					transfer, sessionId);
...
//			}
//			searchResult.addLink(new Link("_specimen", SPECIMEN_DETAIL_BASE_URL
//					+ transfer.getUnitID()));
//
...
//	}
//
//	private void getTaxonForSpecimenFullScientificName(Specimen transfer,
//			SearchResult<Specimen> searchResult, String sessionId)
//	{
...
//						.lookupTaxonForScientificName(identification.getScientificName(), sessionId);
...
//						searchResult.addLink(new Link("_taxon", TAXON_DETAIL_BASE_URL
...
//	}
//
//	private ResultGroupSet<Specimen, String> responseToSpecimenResultGroupSet(
//			SearchResponse response, float minScore, String sessionId)
//	{
//		float maxScore = response.getHits().getMaxScore();
...
//							filteredQuery(
//									matchAllQuery(),
//									boolFilter().must(termFilter(ASSEMBLAGE_ID, assemblageID))
//											.mustNot(
//													termFilter(UNIT_ID + ".raw",
//															transfer.getUnitID())))).execute()
//					.actionGet();
//			SearchHits hits = searchResponse.getHits();
//			for (SearchHit searchHitFields : hits) {
//				ESSpecimen esSpecimenWithSameAssemblageId = getObjectMapper().convertValue(
//						searchHitFields.getSource(), ESSpecimen.class);
//				specimensWithSameAssemblageId.add(SpecimenTransfer
//						.transfer(esSpecimenWithSameAssemblageId));
//			}
//		}
