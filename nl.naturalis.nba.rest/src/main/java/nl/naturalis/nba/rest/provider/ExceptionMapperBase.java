...
public class ExceptionMapperBase implements ExceptionMapper<Throwable> {
	...
	public Response toResponse(Throwable e)
	{
		...
		/*
		 * javax.ws.rs.NotFoundException does not come from our code telling the
		 * client nicely that a query has returned 0 results. It is Wildfly
		 * about to tell the client that the URL cannot be resolved, so this is
		 * rather more deep-level and hard-core.
		 */
		...
		/*
		 * THIS is us telling the client nicely that something could not be
		 * found using the client-provided ID. This is not a "real" error, it's
		 * just REST protocol, so we are not going to present the client with
		 * stack traces.
		 */
