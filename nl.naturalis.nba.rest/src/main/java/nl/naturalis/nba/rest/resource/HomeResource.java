...
@SwaggerDefinition(
		info = @Info(
        		title = "Netherlands Biodiversity Api", 
        		version = "v2",         		
		...
		basePath = "/v2",
		externalDocs = @ExternalDocs(url = "http://docs.biodiversitydata.nl", value = "Documentation and examples"),
		tags = {
				@Tag(name = "specimen", description = "Access specimens present in the collection"),
				...
				@Tag(name = "multimedia", description = "Access multimedia objects associated with specimens"),
...
public class HomeResource {
	...
	public String show(@Context UriInfo uriInfo)
	{
		try {
			...
			if (in == null) {
				/*
				 * We are inside an ear file that was not built
				 * by our ant scripts; we're most likely running
				 * within Eclipse.
				 */
