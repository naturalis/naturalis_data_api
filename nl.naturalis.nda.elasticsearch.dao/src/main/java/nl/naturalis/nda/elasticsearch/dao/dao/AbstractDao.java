...
public abstract class AbstractDao {
	...
	protected SearchResponse executeExtendedSearch(QueryParams params, List<FieldMapping> fields, String type, boolean highlighting, String sessionId)
	{
	 ...
	 * @return
	 */
	protected SearchResponse executeExtendedSearch(QueryParams params, List<FieldMapping> fields, String type, boolean highlighting,
			QueryAndHighlightFields prebuiltQuery, String sessionId)
	{
		...
		Map<String, HighlightBuilder.Field> highlightFields = prebuiltQuery == null || prebuiltQuery.getHighlightFields() == null
				|| prebuiltQuery.getHighlightFields().isEmpty() ? new HashMap<String, HighlightBuilder.Field>() : prebuiltQuery.getHighlightFields();

		for (String nestedPath : nestedFields.keySet()) {
			extendQueryWithNestedFieldsWithSameNestedPath(nonPrebuiltQuery, operator, nestedPath, nestedFields.get(nestedPath), highlightFields,
					highlighting);
		...
		SearchRequestBuilder searchRequestBuilder = newSearchRequest().setTypes(type).setQuery(filteredQuery(completeQuery, geoShape))
	...
	private boolean extractRangeQuery(QueryParams params, BoolQueryBuilder boolQueryBuilder, boolean atLeastOneFieldToQuery)
	{
		if (params.containsKey("gatheringEvent.dateTimeBegin") || params.containsKey("gatheringEvent.dateTimeEnd")) {
			extendQueryWithRangeFilter(boolQueryBuilder, params, "gatheringEvent.dateTimeBegin", "gatheringEvent.dateTimeEnd");
			atLeastOneFieldToQuery = true;
		}

		if (params.containsKey("gatheringEvents.dateTimeBegin") || params.containsKey("gatheringEvents.dateTimeEnd")) {
			extendQueryWithRangeFilter(boolQueryBuilder, params, "gatheringEvents.dateTimeBegin", "gatheringEvents.dateTimeEnd");
			atLeastOneFieldToQuery = true;
		}
	...
	private void extendQueryWithRangeFilter(BoolQueryBuilder boolQueryBuilder, QueryParams params, String dateTimeBegin, String dateTimeEnd)
	{
		String begin = params.getParam(dateTimeBegin);
		String end = params.getParam(dateTimeEnd);
		if (begin != null) {
			RangeFilterBuilder dateTimeBeginRangeFilterBuilder = rangeFilter(dateTimeBegin).from(begin);
	...
	private NestedFilterBuilder createGeoShapeFilter(String geoShape)
	{
		...
		if (shapeBuilder != null) {
			return nestedFilter("gatheringEvent.siteCoordinates",
					geoShapeFilter("gatheringEvent.siteCoordinates.point", shapeBuilder, ShapeRelation.WITHIN));
		}
		return null;
	}
	...
	private void extendQueryWithNestedFieldsWithSameNestedPath(BoolQueryBuilder boolQueryBuilder, Operator operator, String nestedPath,
			List<FieldMapping> fields, Map<String, HighlightBuilder.Field> highlightFields, boolean highlight)
	{
	...
	private void extendQueryWithQuery(BoolQueryBuilder boolQueryBuilder, Operator operator, QueryBuilder nameResolutionQuery)
	{
		if (operator == AND) {
	 ...
	 * @param sessionId
	 */
	protected QueryAndHighlightFields buildNameResolutionQuery(List<FieldMapping> fields, String simpleSearch, BioportalTaxonDao taxonDao,
			boolean highlight, Operator operator, String sessionId)
	{
		if (!hasFieldWithTextWithOneOfNames(fields, IDENTIFICATIONS_VERNACULAR_NAMES_NAME, IDENTIFICATIONS_DEFAULT_CLASSIFICATION_KINGDOM,
		...
		SearchResultSet<Taxon> nameResTaxons = taxonDao.searchReturnsResultSet(nameResTaxonQueryParams, null, null, true, sessionId); // no
																																		// field
																																		// filtering
		...
		for (SearchResult<Taxon> taxonSearchResult : nameResTaxons.getSearchResults()) {
			...
			addMustQueryWithHighlightSupport(queryAndHighlightFields, scientificNameQuery, highlight, taxon.getValidName().getGenusOrMonomial(),
					IDENTIFICATIONS_SCIENTIFIC_NAME_GENUS_OR_MONOMIAL);
			addMustQueryWithHighlightSupport(queryAndHighlightFields, scientificNameQuery, highlight, taxon.getValidName().getSpecificEpithet(),
					IDENTIFICATIONS_SCIENTIFIC_NAME_SPECIFIC_EPITHET);
			addMustQueryWithHighlightSupport(queryAndHighlightFields, scientificNameQuery, highlight, taxon.getValidName().getInfraspecificEpithet(),
	...
	private void addMustQueryWithHighlightSupport(QueryAndHighlightFields highlightFieldsContainer, BoolQueryBuilder query, boolean highlight,
	 ...
	 */
	protected void evaluateSimpleSearch(QueryParams params, Set<String> searchFieldNames, Set<String> simpleSearchFieldNameExceptions)
	{
