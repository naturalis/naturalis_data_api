...
public class MimeTypeCacheFactory {
	...
	private MimeTypeCacheFactory()
	{
		...
		String propVal = System.getProperty(propName, "array");
		if (propVal.equalsIgnoreCase("array")) {
			...
			String fmt = "Invalid value for mimetypecache.type: \"%s\". Valid cache types: \"array\" (default), \"map\"";
	...
	{
		String s = propVal.equalsIgnoreCase("array") ? propVal + " (default)" : propVal;
		logger.info(String.format("Creating mime type cache. Type of cache: \"%s\"", s));
		s = propVal.equalsIgnoreCase("array") ? "map" : "array";
